...
class SIARN3(nn.Module):
    ...
    def __init__(self, embed_dim, hidden_dim, vocab_size, seq_len):
        ...
        self.Wa = nn.Parameter(torch.zeros(3 * embed_dim))
        self.ba = nn.Parameter(torch.zeros(1))

        self.idxs = torch.cartesian_prod(torch.tensor(range(0, seq_len)), torch.tensor(range(0, seq_len)), torch.tensor(range(0, seq_len)))
    ...
    def intra_attention(self, embed):
        ...
        pair_concats = torch.cat([embed[:, self.idxs, :]], dim=1).view(embed.shape[0], -1, 3 * embed.shape[2])
        s1 = torch.matmul(pair_concats, self.Wa)
        s1 = s1.view(embed.shape[0], self.seq_len, self.seq_len, self.seq_len)
        # keep i=j at 0 because the intra-word attention should not represent the same word
        for i in range(0, self.seq_len):
            s1[:, i, i, i] = 0.
    ...
    def forward(self, tweet):
        ...
        a = F.softmax(torch.max(torch.max(s, 2)[0],1)[0], 0)
